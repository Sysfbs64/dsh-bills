  ...
  factory: (require) => {
    ...
    function BillsPage(props) {
      ...
      const load = React.useCallback(() => {
        ...
        Promise.all([apiGet('/api/dsh-bills/summary'), apiGet('/api/dsh-bills/charts')]).then(
          (rs) => { setState({ loading: false, data: rs[0], error: null }); setCharts(rs[1]) },
      ...
      return React.createElement('div', { className: 'bills-page' },
        ...
        React.createElement('div', { className: 'bills-foot' },
          ...
          React.createElement('br', null),
          '· 账单缓存：' + (dbg && dbg.cachePath ? dbg.cachePath : (d.cachePath || '工作区 .bills-cache.json')) + '；增量同步：启动基线 + 事件实时增量 + 60 秒轮询增量（历史会话不重读），打开本页直接读取缓存。',
