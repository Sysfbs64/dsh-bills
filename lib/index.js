...
// 水位线轮询，非 live 历史会话永不重读）；磁盘缓存常驻于用户可配置目录
// （默认 $DSH_HOME/profiles），读写直接走 node:fs，不经 fs 沙箱。
...
// 缓存结构版本：数据结构或计价规则变更时递增。版本不符的旧缓存会被忽略，
// 并触发一次全量基线重算，使历史调用按新规则重新计价（不追溯改价的语义由
// 价目表的 since 档位保证，而不是靠缓存）。
// v5：新增 DeepSeek-V4.1 Flash（deepseek-flash）与 2026-09-10 12:00 降价档、
//     deepseek-v4-pro 自 2026-09-14 12:00 起路由到 Flash 计价。
const CACHE_VERSION = 5

// ---------------------------------------------------------------------------
// 定价表（人民币 / 百万 tokens）
// - DeepSeek：官方人民币价目表（api-docs.deepseek.com/zh-cn/quick_start/pricing）。
//   峰谷定价于 2026-08-17 00:00（北京时间）生效；高峰 = 北京时间（周一至周五）
//   09-12、14-18 点（UTC 01-04、06-10），空闲 = 高峰的一半；自 2026-08-23 00:00
//   （北京时间）起周末（周六、周日）全天为低谷价。生效前的调用按当时官方价
//   计费，不追溯。
//   · 2026-09-10 12:00（北京时间）：V4.1 Flash 发布并降价，模型名 deepseek-flash
//     与临时路由的旧名 deepseek-v4-flash、deepseek-v4-flash-vision-exp 统一按
//     Flash 新价计费：空闲 0.02 / 1 / 4，高峰 0.04 / 2 / 8。
//   · 2026-09-14 12:00（北京时间）起：deepseek-v4-pro 的请求全部路由到
//     V4.1 Flash，并按 Flash 价格计费（至 V4.1 Pro 上线前）。
// - 其余模型：官方美元价按 1 USD = 6.79 CNY 折算（2026-08 央行中间价 6.7882）；
//   不区分峰谷，单一价档。
//
// 结构：PRICING = [{ match, tiers }]，tiers 为该模型的价格档位，按时间由新到旧
// 排列，取第一个 `since <= ts` 的档位；末档不设 since，作为该模型最早的兜底
// 价档。DeepSeek 各档均含 peak / offPeak 两套单价。
// ---------------------------------------------------------------------------
const DEEPSEEK_SINCE = Date.UTC(2026, 7, 16, 16, 0, 0) // 2026-08-17 00:00（北京时间）峰谷定价生效
const V41_FLASH_SINCE = Date.UTC(2026, 8, 10, 4, 0, 0) // 2026-09-10 12:00（北京时间）V4.1 Flash 降价生效
const V4_PRO_ROUTE_SINCE = Date.UTC(2026, 8, 14, 4, 0, 0) // 2026-09-14 12:00（北京时间）v4-pro 路由至 V4.1 Flash

// 周末全天低谷价生效时间 = 2026-08-23 00:00（北京时间）即 2026-08-22 16:00 UTC
const WEEKEND_OFFPEAK_SINCE = Date.UTC(2026, 7, 22, 16, 0, 0)

// DeepSeek 价档（元 / 百万 tokens）：峰谷定价前的旧价、2026-08-17 峰谷价、2026-09-10 V4.1 Flash 新价
const FLASH_LEGACY = { peak: { cacheHit: 0.02, cacheMiss: 1.0, out: 2.0 }, offPeak: { cacheHit: 0.02, cacheMiss: 1.0, out: 2.0 } }
const FLASH_PEAKVALLEY = { peak: { cacheHit: 0.1, cacheMiss: 3.0, out: 9.0 }, offPeak: { cacheHit: 0.05, cacheMiss: 1.5, out: 4.5 } }
const FLASH_V41 = { peak: { cacheHit: 0.04, cacheMiss: 2.0, out: 8.0 }, offPeak: { cacheHit: 0.02, cacheMiss: 1.0, out: 4.0 } }
const PRO_LEGACY = { peak: { cacheHit: 0.025, cacheMiss: 3.0, out: 6.0 }, offPeak: { cacheHit: 0.025, cacheMiss: 3.0, out: 6.0 } }
const PRO_PEAKVALLEY = { peak: { cacheHit: 0.3, cacheMiss: 9.0, out: 27.0 }, offPeak: { cacheHit: 0.15, cacheMiss: 4.5, out: 13.5 } }

// flash 系列三个模型名（新名 deepseek-flash + 两个旧名路由）共用同一组价档
const FLASH_TIERS = [
  Object.assign({ since: V41_FLASH_SINCE }, FLASH_V41),
  Object.assign({ since: DEEPSEEK_SINCE }, FLASH_PEAKVALLEY),
  FLASH_LEGACY,
]

const PRICING = [
  // 旧名先列：'deepseek-v4-flash-vision-exp' 含 'deepseek-v4-flash' 前缀
  { match: 'deepseek-v4-flash-vision-exp', tiers: FLASH_TIERS },
  { match: 'deepseek-v4-flash', tiers: FLASH_TIERS },
  { match: 'deepseek-flash', tiers: FLASH_TIERS },
  {
    match: 'deepseek-v4-pro',
    tiers: [
      Object.assign({ since: V4_PRO_ROUTE_SINCE }, FLASH_V41), // 路由至 V4.1 Flash，按 Flash 价计费
      Object.assign({ since: DEEPSEEK_SINCE }, PRO_PEAKVALLEY),
      PRO_LEGACY,
    ],
  },
...
// 某模型在某时刻适用的价档：tiers 由新到旧，取首个 since <= ts 的档位；
// 无 since 的末档为兜底。非 DeepSeek 模型没有 tiers，本身就是唯一价档。
function ratesFor(p, ts) {
  const tiers = p.tiers
  if (!tiers || tiers.length === 0) return p
  for (const t of tiers) {
    if (t.since === undefined || ts >= t.since) return t
  }
...
function priceFor(model, ts) {
  ...
  for (const p of PRICING) {
    if (m.indexOf(p.match) !== -1) {
      const r = ratesFor(p, ts)
      return isPeak(ts) ? r.peak : r.offPeak
    }
  }
  return null
}
...
// 价目规则与折叠逻辑外露：供离线复算 / 校验脚本复用同一套实现（宿主加载插件
// 只用 name / inject / apply，附加具名导出无副作用）。
export { PRICING, ratesFor, priceFor, isPeak, beijingParts, freshState, newSessionState, foldEvent, foldEvents, CACHE_VERSION }
...
export function apply(ctx) {
  ...
  const CONFIG_FILE = '.dsh-bills-config.json'
  let cfg = loadCacheConfig()
  let state = null
  ...
  // -------------------------------------------------------------------------
  // 缓存位置：用户可在账单页 UI 中自定义（默认 $DSH_HOME/profiles）。
  // 路径零硬编码：$DSH_HOME 取自框架注入的 ctx.dshHomePath，回退环境变量
  // DSH_HOME，再回退 ~/.dsh。读写直接用 node:fs（宿主侧插件与宿主同权，
  // 不经 ctx.fs 沙箱），配置持久化在 <默认目录>/.dsh-bills-config.json。
  // -------------------------------------------------------------------------
  function dshHomeDir() {
    const hp = ctx && ctx.dshHomePath
    if (hp) {
      try {
        const v = hp()
        if (typeof v === 'string' && v) return v
      } catch (err) { /* 服务不可用：回退环境变量 */ }
    }
    const env = process.env.DSH_HOME
    if (env && env.trim()) return env.trim()
    try { return join(homedir(), '.dsh') } catch (err) { return null }
  }

  function defaultCacheDir() {
    const h = dshHomeDir()
    return h ? join(h, 'profiles') : null
  }

  function configPath() {
    const d = defaultCacheDir()
    return d ? join(d, CONFIG_FILE) : null
  }
  ...
  function loadCacheConfig() {
    try {
      ...
      if (p && existsSync(p)) {
        ...
        if (data && typeof data.cacheDir === 'string' && data.cacheDir.trim() && isAbsolutePath(data.cacheDir.trim())) {
          return { cacheDir: data.cacheDir.trim().replace(/[\\/]+$/, '') }
        }
      }
    } catch (err) { /* 配置缺失或损坏：使用默认目录 */ }
    return { cacheDir: null }
  }

  function effectiveCacheDir() {
    return cfg.cacheDir || defaultCacheDir()
  }

  function cacheFilePath() {
    const d = effectiveCacheDir()
    return d ? join(d, CACHE_FILE) : null
  }
  ...
  // 应用新配置：校验路径 → 迁移旧缓存文件 → 持久化配置 → 切换生效目录
  function applyCacheConfig(value) {
    let nextDir = null
    if (typeof value === 'string' && value.trim()) {
      nextDir = value.trim().replace(/[\\/]+$/, '')
      if (!isAbsolutePath(nextDir)) throw new Error('缓存路径必须是绝对路径')
    } else {
      nextDir = defaultCacheDir()
    }
    if (!nextDir) throw new Error('无法解析缓存目录（$DSH_HOME 不可用）')
    const oldDir = effectiveCacheDir()
    const oldFile = oldDir ? join(oldDir, CACHE_FILE) : null
    const newFile = join(nextDir, CACHE_FILE)
    if (oldFile && oldFile !== newFile && existsSync(oldFile) && !existsSync(newFile)) {
      try {
        mkdirSync(nextDir, { recursive: true })
        copyFileSync(oldFile, newFile)
      } catch (err) {
        throw new Error('迁移旧缓存失败：' + (err && err.message ? err.message : err))
      }
    }
    const cp = configPath()
    if (cp) {
      try {
        ...
        writeFileSync(cp, JSON.stringify(nextDir === defaultCacheDir() ? {} : { cacheDir: nextDir }, null, 2), 'utf8')
      } catch (err) {
        throw new Error('保存配置失败：' + (err && err.message ? err.message : err))
      }
    }
    cfg = { cacheDir: nextDir === defaultCacheDir() ? null : nextDir }
    activeCachePath = existsSync(newFile) ? newFile : activeCachePath
    return configInfo()
  }
  ...
  // 直写磁盘（node:fs）：目录自建，JSON 全量覆写；损坏时下次启动全量重建，自愈。
  function writeCacheSync() {
    const p = cacheFilePath()
    if (!p || !state) return
    try {
      mkdirSync(dirname(p), { recursive: true })
      writeFileSync(p, JSON.stringify(serializeState()), 'utf8')
      activeCachePath = p
      lastCacheError = null
    } catch (err) {
  ...
  function loadDisk() {
    ...
    try {
      ...
      const data = JSON.parse(readFileSync(p, 'utf8'))
      if (data && data.version === CACHE_VERSION && data.sessions && data.global) {
        if (!state) {
          state = freshState()
  ...
  function deriveSummary() {
    ...
    return {
      ...
      pricing: {
        ...
        peakLabel: 'DeepSeek 高峰时段 = 北京时间（周一至周五）09:00–12:00、14:00–18:00（UTC 01:00–04:00、06:00–10:00）；自 2026-08-23（北京时间）起周末（周六、周日）全天为低谷价',
        latestLabel: 'DeepSeek-V4.1 Flash 降价于 2026-09-10 12:00（北京时间）生效：deepseek-flash（连同按 Flash 计费的旧名 deepseek-v4-flash、deepseek-v4-flash-vision-exp）空闲 0.02 / 1 / 4 元、高峰 0.04 / 2 / 8 元；自 2026-09-14 12:00（北京时间）起 deepseek-v4-pro 的请求路由至 V4.1 Flash 并按 Flash 价格计费',
  ...
  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-bills/config',
    handler: async (req, res) => {
      if (req.method === 'POST') {
        try {
          const raw = await readBody(req)
          const body = raw ? JSON.parse(raw) : {}
          const info = applyCacheConfig(body && body.cacheDir !== undefined ? body.cacheDir : null)
          return json(res, 200, info)
        } catch (err) {
          return json(res, 400, { error: String(err && err.message ? err.message : err) })
        }
      }
      json(res, 200, configInfo())
    },
  })
  ctx.webServer.register({
    kind: 'exact',
